...
classes = ['enojo','felicidad','neutral','sorpresa','tristeza']
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		(Xi, Yi, Xf, Yf) = box
		(enojo, felicidad, neutral, sorpresa, tristeza) = pred


		label = ''
		# Se agrega la probabilidad en el label de la imagen
		label = "{}: {:.0f}%".format(classes[np.argmax(pred)], max('enojo', 'felicidad', 'neutral', 'sorpresa', 'tristeza') * 100)
		...
		plt.clf()
		y = [enojo, felicidad, neutral, sorpresa, tristeza]
		plt.xticks(x, ['enojo', 'felicidad', 'neutral', 'sorpresa', 'tristeza'])
		plt.grid(True)
		plt.ylim([0.0,1.0])
